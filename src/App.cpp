...
{
    {
        std::lock_guard<std::mutex> lock(mtxApp);  // Lock to avoid race conditions when accessing client_sockets
    ...
    {
        std::lock_guard<std::mutex> lock(mtxApp);  // Lock to avoid race conditions when modifying client_sockets
...
void App::acceptMultipleClients(int server_fd, struct sockaddr_in &address)
{
    ...
    while (true)
    {
        ...
        threads.emplace_back(&App::handleClient, this, new_socket);  // Create a new thread for the client
    }
}
