...
{
    manager.addUser(101);
    std::vector<unsigned long int> movies = {1, 2, 3}; // setting up the movies set
...
{
    manager.addUser(101);
    std::vector<unsigned long int> movies = {1, 2}; // setting up the movies set
    manager.addMovies(101, movies);
...
TEST_F(MovieRecommenderTest, ReturnsRecommendations)
{
    ...
    std::vector<unsigned long int> recommendations = manager.recommendMovies(101, 104);
...
TEST_F(MovieRecommenderTest, ReturnsRecommendations_SecondTest)
{
    ...
    std::vector<unsigned long int> recommendations = manager.recommendMovies(1, 104);

    std::vector<unsigned long int> expected = {105, 106, 111, 110, 112, 113, 107, 108, 109, 114};
...
{
    manager.addUser(101);
    std::vector<unsigned long int> movies = {1, 2, 3};
...
TEST_F(MovieRecommenderTest, Recommend_MaximumLimit)
{
    manager.addUser(101);
    manager.addMovies(101, {100, 101, 102});

    for (unsigned long int i = 0; i < 15; ++i) // Add 15 users with overlapping movies
    {
        manager.addUser(200 + i);
        manager.addMovies(200 + i, {100 + i});
    }
